...
let clothing_container = document.createElement("div");
clothing_container.classList.add("row", "justify-center", "card_container");

let accessories_container = document.createElement("div");
accessories_container.classList.add("row", "justify-center", "card_container");
...
for (let i = 0; i < productList.length; i++) {
    ...
    if (productList[i]["isAccessory"])
        card.id = "accessories_" + productList[i]["id"];
    else card.id = "clothing_" + productList[i]["id"];

    // card link - a
    element = document.createElement("a");
    element.classList.add("column");
    element.href = "product.html?product_id=" + productList[i]["id"];
    card.appendChild(element);

    // image container div
    element = document.createElement("div");
    element.classList.add("image");
    card.querySelector("a").appendChild(element);

    // image
    element = document.createElement("img");
    element.src = productList[i]["preview"];
    card.querySelector(".image").appendChild(element);

    // details container div
    element = document.createElement("div");
    element.classList.add("column", "details");
    card.querySelector("a").appendChild(element);

    // card title h3
    element = document.createElement("h3");
    element.classList.add("title");
    element.appendChild(document.createTextNode(productList[i]["name"]));
    card.querySelector(".details").appendChild(element);

    // card subtitle h4
    element = document.createElement("h4");
    element.classList.add("subtitle");
    element.appendChild(document.createTextNode(productList[i]["brand"]));
    card.querySelector(".details").appendChild(element);

    // card price h5
    element = document.createElement("h5");
    element.classList.add("price");
    element.appendChild(document.createTextNode("Rs " + productList[i]["price"]));
    card.querySelector(".details").appendChild(element);

    console.log(card);

    if (productList[i]["isAccessory"]) accessories_container.appendChild(card);
    else clothing_container.appendChild(card);
}

document.getElementById("clothing").appendChild(clothing_container);
document.getElementById("accessories").appendChild(accessories_container);
